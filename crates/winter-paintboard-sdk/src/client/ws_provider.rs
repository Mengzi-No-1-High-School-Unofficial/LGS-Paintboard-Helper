...
impl WsProvider {
                                        ...
                                        if let Ok(protocol_msg) = ProtocolMessage::parse(&data) {
                                            match protocol_msg {
                                                ProtocolMessage::HeartbeatPing => {
                                                    ...
                                                    let pong_msg =
                                                        vec![OpCode::HeartbeatPong as u8];
                                                    trace!("发送心跳 PONG 响应");
                                                    let mut send_conn_guard =
                                                        connection_clone.lock().await;
                                                    if let Some(ref mut ws_stream) =
                                                        *send_conn_guard
                                                    {
                                                        match ws_stream
                                                            .send(Message::Binary(pong_msg))
                                                            .await
                                                        {
                                                            Ok(_) => {
                                                                trace!("心跳 PONG 发送成功");
                                                ...
                                                ProtocolMessage::PaintResult {
                                                    drawing_id,
                                                    status,
                                                } => {
                                                    trace!(
                                                        "收到绘图结果，drawing_id: {}, status: {}",
                                                        drawing_id,
                                                        status
                                                    );
                                                    ...
                                                    let mut channels_guard =
                                                        response_channels_clone.lock().await;
                                                    // Take the first key
                                                    let first_key =
                                                        channels_guard.keys().next().cloned();
                                                    if let Some(key) = first_key {
                                                        // Remove the key-value pair from the map
                                                        if let Some(tx) =
                                                            channels_guard.remove(&key)
                                                        {
    ...
    pub async fn paint(&mut self, pos: Pos, color: Rgb) -> Result<PaintResult, PaintboardError> {
        ...
        let uid = self
            .uid
            .ok_or(PaintboardError::auth("UID not set".to_string()))?;
        let token = self
            .token
            .clone()
            .ok_or(PaintboardError::auth("Token not set".to_string()))?;
        ...
        // Send the message
        {
            ...
            match conn_guard.as_mut() {
                ...
                None => {
                    ...
                    // 发送错误事件到事件总线
                    let event_bus = EventBus::global();
                    let _ = event_bus.send(Event::error_event(
                        "Connection not available for sending paint message".to_string(),
                    ));
    ...
    pub async fn paint_batch(
        ...
        match self.rate_limiter.check() {
            ...
            Err(_) => {
                // 超过速率限制，静默丢弃请求并记录 debug 日志
                debug!(
                    "速率限制：paint_batch 请求被丢弃，超过每秒120个包的限制 (操作数量: {})",
                    ops_count
                );
                return Ok(());
            }
        ...
        for (pos, color) in &operations {
            // Use reference to avoid moving operations
            // Generate a unique paint ID
            let paint_id = rand::random::<u64>();
        ...
        // Send all data in one sticky packet
        {
            ...
            match conn_guard.as_mut() {
                ...
                None => {
                    ...
                    let event_bus = EventBus::global();
                    let _ = event_bus.send(Event::error_event(
                        "Connection not available for sending batch message".to_string(),
                    ));
    ...
    pub async fn send_heartbeat_pong(&mut self) -> Result<(), PaintboardError> {
        ...
        {
            let mut conn_guard = self.connection.lock().await;
            let ws_stream = conn_guard
                .as_mut()
                .ok_or(PaintboardError::ConnectionClosed)?;
