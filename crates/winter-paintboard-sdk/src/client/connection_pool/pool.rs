...
impl ConnectionPool {
        ...
        T: Send,
    {
        ...
        if self.circuit_breaker.is_open() {
            return Err(PaintboardError::Internal("Circuit breaker is open".to_string()));
        }
    ...
    fn categorize_error(&self, error: &PaintboardError) -> &'static str {
        match error {
            ...
            PaintboardError::InvalidData => "invalid_data",
            ...
            pool.update_metrics(|m| {
                match error_type.as_str() {
                    ...
                    s if s.starts_with("http_") => {
                        if s.contains("4") && !s.contains("5") {  // 客户端错误
                            m.client_errors += 1;
                        } else if s.contains("5") {  // 服务器错误
                            m.server_errors += 1;
                        } else {
