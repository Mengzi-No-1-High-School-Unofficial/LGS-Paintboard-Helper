...
use crate::{
    ...
    BasicClient, PaintboardClientTrait,
};

use super::pool::{ConnectionGuard, ConnectionPool};
...
impl PoolClient {
    ...
    pub async fn cleanup_inactive_connections(&self, max_idle_time: Duration) {
        self.write_pool
            .cleanup_inactive_connections(max_idle_time)
            .await;
    }
...
impl PaintboardClientTrait for PoolClient {
    ...
    async fn paint_batch(&mut self, operations: Vec<(Pos, Rgb)>) -> Result<(), PaintboardError> {
        // 增加批量请求计数
        self.write_pool
            .increment_batch_requests(operations.len() as u64)
            .await;
