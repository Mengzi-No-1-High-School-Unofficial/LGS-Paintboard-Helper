...
impl WriteOnlyPool {
    ...
    pub async fn get(&self) -> Result<deadpool::managed::Object<WriteOnlyManager>, PaintboardError> {
        tracing::debug!("当前可用的连接数：{}, 连接池大小：{}", self.pool.status().available, self.pool.status().size);
