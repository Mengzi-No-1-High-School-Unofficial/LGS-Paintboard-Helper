use crate::{
    ...
    models::{AuthRequest, Board, TokenResponse},
};
use std::sync::Arc;
...
impl HttpProvider {
    ...
    pub async fn get_board(&self) -> Result<Board, PaintboardError> {
        ...
        if bytes.len() != 1_800_000 {
            return Err(PaintboardError::invalid_data(format!(
                "Board data size mismatch: expected 1,800,000 bytes, got {}",
                bytes.len()
            )));
        }
        ...
        let board = tokio::task::spawn_blocking(move || Board::from_bytes(bytes_vec))
            .await
    ...
    pub async fn get_token(&self, uid: u32, access_key: &str) -> Result<String, PaintboardError> {
        ...
        if token_response.status_code != 200 {
            return Err(PaintboardError::auth(format!(
                "API returned error status {}: token data",
